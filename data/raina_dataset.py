...
class RainaDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        self.dir_H = os.path.join(opt.dataroot, opt.phase + 'H')
        self.dir_I = os.path.join(opt.dataroot, opt.phase + 'I')
        ...
        if opt.phase == "test" and not os.path.exists(self.dir_A) \
           and os.path.exists(os.path.join(opt.dataroot, "valA")):
            ...
            self.dir_I = os.path.join(opt.dataroot, "valI")
    ...
    def __getitem__(self, index):
        ...
        H_path = self.H_paths[index_A]
        I_path = self.I_paths[index_A]
        ...
        fix_transform_seg = get_transform(self.opt, transform_params, segmentation=True, convert=False)
        fix_transform_dpt = get_transform(self.opt, transform_params, depth=False)
        ...
        H = fix_transform_dpt(H_img)
        I = fix_transform_seg(I_img)

        return {'A': A, 'B': B, 'C': C, 'D1': D1, 'D2': D2, 'D3': D3, 'E1': E1, 'E2': E2, 'H': H, 'A_paths': A_path, 'B_paths': B_path, 'C_paths': C_path, 'D1_paths': D1_path,'D2_paths': D2_path, 'D3_paths': D3_path,'E1_paths': E1_path, 'E2_paths': E2_path, 'H_paths': H_path, 'I': I, 'I_paths': I_path}
    ...
    def __len__(self):
        ...
        """
        return max(self.A_size, self.B_size, self.C_size, self.D_size, self.E_size, self.H_size)
