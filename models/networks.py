...
def get_scheduler(optimizer, opt):
    ...
    if opt.lr_policy == 'linear':
        ...
        scheduler = lr_scheduler.CosineAnnealingLR(optimizer, T_max=opt.n_epochs, eta_min=1e-6)#!eta_min=0
    else:
...
def define_R(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
...
class CBD(nn.Module):
    """CBD (Controllable Blind Decomposition) Network
    Based on Restormer architecture with modifications for controllable multi-degradation removal.
    """
    def __init__(self, num_blocks=[4, 6, 6, 8], num_heads=[1, 2, 4, 8], channels=[36, 72, 144, 288], num_refinement=4,
        ...
        self.output = nn.Sequential(
                nn.Conv2d(channels[1], 96, kernel_size=3, padding=1, bias=False),
                nn.BatchNorm2d(96),
        ...
        self.classification = nn.Sequential(
                nn.Conv2d(channels[-1], 256, kernel_size=3, stride=1, padding=1, bias=False),
                nn.BatchNorm2d(256),
                nn.ReLU(inplace=True),
        ...
        self.pr = nn.Sequential(
                nn.Linear(10,24*24*channels[-1]),
                nn.Unflatten(0,(channels[-1],24,24)),
            )
        
        self.pr2 = nn.Sequential(
                nn.Linear(10,channels[-1]),
                nn.Unflatten(0,(channels[-1],1,1)),
            )

        self.pr3 = nn.Sequential(
                nn.Linear(10,channels[-2]),
                nn.Unflatten(0,(channels[-2],1,1)),
            )

        self.pr4 = nn.Sequential(
                nn.Linear(10,channels[-3]),
                nn.Unflatten(0,(channels[-3],1,1)),
            )
    ...
    def forward(self, x, prs1=None, prs2=None):
        ...
        label = self.classification(out_enc4)
        outList1 = []
        outList2 = []

        if prs1 is not None:
            prList = [self.pr(pr.cuda()) for pr in prs1]
            prList2 = [self.pr2(pr.cuda()) for pr in prs1]
            prList3 = [self.pr3(pr.cuda()) for pr in prs1]
            prList4 = [self.pr4(pr.cuda()) for pr in prs1]
            prList5 = [self.pr5(pr.cuda()) for pr in prs1]
            
            for i in range(len(prList)):
                out_enc4 = (out_enc4 + prList[i]) * prList2[i]
                out_dec3 = self.decoders[0](self.reduces[0](torch.cat([self.ups[0](out_enc4), out_enc3], dim=1))) * prList3[i]
                out_dec2 = self.decoders[1](self.reduces[1](torch.cat([self.ups[1](out_dec3), out_enc2], dim=1))) * prList4[i]
                fd = self.decoders[2](torch.cat([self.ups[2](out_dec2), out_enc1], dim=1)) * prList5[i]

                fr = self.refinement(fd)
                outList1.append(self.output(fr))
        
        if prs2 is not None:
            prList = [self.pr(pr.cuda()) for pr in prs2]
            prList2 = [self.pr2(pr.cuda()) for pr in prs2]
            prList3 = [self.pr3(pr.cuda()) for pr in prs2]
            prList4 = [self.pr4(pr.cuda()) for pr in prs2]
            prList5 = [self.pr5(pr.cuda()) for pr in prs2]
            ...
            for i in range(len(prList)):
                ...
                fr = self.refinement(fd)
                outList2.append(self.output2(fr))

        return outList1, outList2, label
